...
function pushLogs(label) {
  try {
    git(['add', 'automatyzacje/pz/run-log.jsonl', 'automatyzacje/pz/state']);

    const status = git(['status', '--porcelain', '--', 'automatyzacje/pz/run-log.jsonl', 'automatyzacje/pz/state']);
    if (!status.trim()) {
      console.log('[git-log-push] Brak zmian w logu/stanie — nic do commitowania.');
      return;
    }

    git(['commit', '-m', 'Log automatyzacji PZ' + (label ? ' — ' + label : '')]);
    git(['push']);
